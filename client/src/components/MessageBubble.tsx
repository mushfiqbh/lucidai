...
export const MessageBubble: React.FC<MessageBubbleProps> = ({ message, isStreaming = false }) => {
  ...
  return (
    <div className={`flex gap-4 p-4 ${isUser ? 'justify-end' : 'justify-start'} group animate-fade-in`}>
      {!isUser && (
      ...
      <div className={`max-w-4xl ${isUser ? 'order-first' : ''}`}>
                ...
                <div className={`prose prose-sm max-w-none ${isUser ? 'prose-invert' : ''}`}>
                  {isUser ? (
                    ...
                    <div className="relative">
                      <MarkdownRenderer content={content.text} isUser={isUser} />
                      {isStreaming && (
        ...
        <div className={`flex items-center gap-2 mt-2 text-xs text-gray-500 ${isUser ? 'justify-end' : 'justify-start'}`}>
          ...
          {!isUser && !isStreaming && message.content.some(c => c.type === 'text') && (
            <button
              onClick={() => {
